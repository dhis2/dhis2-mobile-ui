...
fun InputPositiveIntegerOrZero(
    ...
    InputShell(
        modifier = modifier.testTag("INPUT_POSITIVE_INTEGER_OR_ZERO"),
        isRequiredField = isRequiredField,
        title = title,
        primaryButton = {
            if (deleteButtonIsVisible) {
                IconButton(
                    modifier = Modifier.testTag("INPUT_POSITIVE_INTEGER_OR_ZERO_RESET_BUTTON"),
                    icon = {
                        Icon(
        ...
        legend = {
            legendData?.let {
                Legend(legendData, Modifier.testTag("INPUT_POSITIVE_INTEGER_OR_ZERO_LEGEND"))
            }
        },
        supportingText = {
            supportingText?.forEach {
                ...
                SupportingText(
                    label.text,
                    label.state,
                    modifier = Modifier.testTag("INPUT_POSITIVE_INTEGER_OR_ZERO_SUPPORTING_TEXT"),
                )
            }
        },
        inputField = {
            BasicInput(
                modifier = Modifier.testTag("INPUT_POSITIVE_INTEGER_OR_ZERO_FIELD"),
